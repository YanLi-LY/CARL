...
class LossNetwork(torch.nn.Module):
    ...
    def forward(self, dehaze, gt, input1, input2, other_method):
        loss = []
        ...
        for i in range(len(dehaze_features)):
            ...
            dehaze_gt = torch.exp(torch.div(-self.L1(dehaze_features[i], gt_features[i]), 0.5))
            dehaze_input1 = torch.exp(torch.div(-self.L1(dehaze_features[i], input1_features[i]), 0.5))
            dehaze_input2 = torch.exp(torch.div(-self.L1(dehaze_features[i], input2_features[i]), 0.5))
            dehaze_other_method = torch.exp(torch.div(-self.L1(dehaze_features[i], other_method_features[i]), 0.5))

            per_loss = -torch.log(dehaze_gt / (dehaze_gt + dehaze_input1 + dehaze_input2 + dehaze_other_method))
